...
class ANTServices: NSObject {

    static let sharedInstance = ANTServices()
...
public func searchArticles(query: String, completion: @escaping ([SearchArticle]) -> ()) {
...
public func fetchFeaturedArticles(vc:UIViewController!, completion: @escaping ([FeaturedArticle]) -> ()) {
    let urlComponents = UrlComponents(path: "mostpopular/v2/mostviewed/all-sections/30.json")
    let request = URLRequest(url: urlComponents.url)
    
    let progressHUD = MBProgressHUD.showAdded(to: vc.view, animated: true)
